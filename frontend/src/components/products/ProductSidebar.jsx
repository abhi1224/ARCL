      ...
      <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-8">
        ...
        <div>
          <div className="flex justify-between items-center border-b-2 py-2">
            ...
            <span className="h-6 w-6 rounded-full bg-[#021C57] text-white flex justify-center items-center">
          ...
          <div className="space-y-2">
            <div className="overflow-y-auto max-h-64 my-5">
              {equipmentTypes.map((item) => (
                <button
                  key={item._id}
                  onClick={() => setSelectedEquipmentType(item.slug)}
                  className={`
                  w-full
                  text-left
                  px-4
          ...
          <div className=" space-y-2 overflow-y-auto max-h-96 my-5 ">
            ...
            {categories.map((item) => (
              <Link
                ...
                className={` w-full block text-left px-4 py-3 rounded-xl transition ${selectedCategory === item.slug ? "bg-blue-50 text-[#021C57] border-l-4 border-[#021C57]" : "hover:bg-gray-50"} `}
              >
